...
{
    /// <summary>
    /// Composite vidéo overlay (éclair gauche) sur le buffer LED.
    /// Authoring alimente les pixels via EclaireGaucheOverlayRenderer ;
    /// Routing appelle CompositeOnto sans dépendre de Authoring.
    /// </summary>
    public static class EclaireGaucheOverlayCompositor
    {
        private const float LumaThreshold = 0.10f;
        private const float LumaSoftness = 0.08f;
        private const float LumaBoost = 3.0f;
        private const float BrightBoost = 1.8f;
        ...
        public static void CompositeOnto(Color32[] buffer, int screenWidth, int screenHeight)
        {
            ...
            // Plein cadre : la vidéo occupe tout l'écran
            int offsetX = 0;
            int offsetY = 0;
