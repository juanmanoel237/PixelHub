...
{
    /// <summary>
    /// Composite vidéo overlay (flash) sur le buffer LED.
    /// Authoring alimente les pixels via FlashOverlayRenderer ;
    /// Routing appelle CompositeOnto sans dépendre de Authoring.
    /// </summary>
    public static class FlashOverlayCompositor
    {
        private const float LumaThreshold = 0.10f;
        private const float LumaSoftness = 0.08f;
        private const float LumaBoost = 3.0f;   // multiplie la luma pour que le flash ressorte
        ...
        public static void CompositeOnto(Color32[] buffer, int screenWidth, int screenHeight)
        {
            ...
            for (int y = 0; y < drawH; y++)
            {
                ...
                for (int x = 0; x < drawW; x++)
                {
                    ...
                    // Boost brightness pour que le flash soit bien visible sur les LEDs
                    src = new Color32(
