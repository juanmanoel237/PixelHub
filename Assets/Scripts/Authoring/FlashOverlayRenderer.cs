...
{
    /// <summary>
    /// Gère le VideoPlayer pour la vidéo de flash overlay et expose la RenderTexture.
    /// Alimente FlashOverlayCompositor pour envoyer le flash au mur LED.
    /// </summary>
    public class FlashOverlayRenderer : MonoBehaviour
    {
        public static FlashOverlayRenderer Instance { get; private set; }
        ...
        private void OnDisable()
        {
            ...
            FlashOverlayCompositor.ClearFrame();
        ...
        private void OnReadbackComplete(AsyncGPUReadbackRequest request)
        {
            ...
            if (_frameCount % 60 == 0)
            {
                ...
                Debug.Log($"[FlashOverlayRenderer] Frame {_frameCount}, Max Luma: {maxLuma}");
            }
            _frameCount++;

            FlashOverlayCompositor.SetFrame(_pixelBuffer, _pixelWidth, _pixelHeight);
        }
        ...
        private void InitVideoPlayer()
        {
            ...
            _videoPlayer.url = ResolveVideoUrl("flash_overlay");
            ...
            Debug.Log("[FlashOverlayRenderer] Préparation de la vidéo flash…");
        }
        ...
        private void OnPrepared(VideoPlayer vp)
        {
            ...
            if (!_paused)
                vp.Play();
            _videoReady = true;
            Debug.Log($"[FlashOverlayRenderer] Vidéo flash prête ({w}×{h}), lecture en boucle.");
        }

        private void OnError(VideoPlayer vp, string message)
        {
            Debug.LogWarning(
                "[FlashOverlayRenderer] Vidéo flash illisible. " +
                "Placez flash_overlay.webm (VP8+alpha) ou .mp4 (H.264) dans StreamingAssets. " +
                $"Détail : {message}");
        }
