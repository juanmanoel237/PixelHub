...
{
    /// <summary>
    /// Gère le VideoPlayer pour la vidéo éclair droite et expose la RenderTexture.
    /// Alimente EclaireDroiteOverlayCompositor pour envoyer l'éclair au mur LED.
    /// </summary>
    public class EclaireDroiteOverlayRenderer : MonoBehaviour
    {
        public static EclaireDroiteOverlayRenderer Instance { get; private set; }
        ...
        private void Start() { InitVideoPlayer(); }
        ...
        {
            EclaireDroiteOverlayCompositor.ClearFrame();
        ...
        private void OnReadbackComplete(AsyncGPUReadbackRequest request)
        {
            ...
            // Guard : ignore le callback si l'objet a été désactivé pendant le readback
            ...
            if (_frameCount % 60 == 0)
            {
                ...
                Debug.Log($"[EclaireDroiteOverlayRenderer] Frame {_frameCount}, Max Luma: {maxLuma}");
            }
            _frameCount++;

            EclaireDroiteOverlayCompositor.SetFrame(_pixelBuffer, _pixelWidth, _pixelHeight);
        }
        ...
        public void SetPaused(bool paused)
        {
            ...
            if (paused) { if (_videoPlayer.isPlaying) _videoPlayer.Pause(); }
            else if (!_videoPlayer.isPlaying) { _videoPlayer.Play(); }
        }
        ...
        private void InitVideoPlayer()
        {
            ...
            _videoPlayer.url = ResolveVideoUrl("eclaire_droite_overlay");
            ...
            Debug.Log("[EclaireDroiteOverlayRenderer] Préparation de la vidéo éclair droite…");
        }
        ...
        private void OnPrepared(VideoPlayer vp)
        {
            ...
            if (!_paused) vp.Play();
            _videoReady = true;
            Debug.Log($"[EclaireDroiteOverlayRenderer] Vidéo prête ({w}×{h}), lecture en boucle.");
        }

        private void OnError(VideoPlayer vp, string message)
        {
            Debug.LogWarning(
                "[EclaireDroiteOverlayRenderer] Vidéo éclair droite illisible. " +
                "Placez eclaire_droite_overlay.webm (VP8+alpha) ou .mp4 (H.264) dans StreamingAssets. " +
                $"Détail : {message}");
        }
